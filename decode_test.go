...
func TestDelimiterTabIsReadFromTheHeader(t *testing.T) {
	...
	src := "rows[1\t]{a\tb\tc}:\n  one\ttwo\tthree\n"
	doc, err := DecodeTabular(strings.NewReader(src))
	if err != nil {
...
func TestExplicitCommaDelimiterStillDecodes(t *testing.T) {
	...
	doc, err := DecodeTabular(strings.NewReader("rows[1,]{a,b,c}:\n  one,two,three\n"))
	if err != nil {
...
func TestNullAndBooleanTokensProject(t *testing.T) {
	...
	doc, err := DecodeTabular(strings.NewReader("r[1|]{n|e|t|f}:\n  null||true|false\n"))
	if err != nil {
		t.Fatalf("DecodeTabular: %v", err)
	}
	row := doc.Rows[0]
...
func TestUnquotedPaddingIsTrimmedAndQuotedPaddingSurvives(t *testing.T) {
	...
	// is a lie.
	doc, err := DecodeTabular(strings.NewReader("r[1|]{bare|quoted}:\n  pad me  |\"  keep me  \"\n"))
	if err != nil {
		t.Fatalf("DecodeTabular: %v", err)
	}
...
func TestAnUnknownEscapeIsAnError(t *testing.T) {
	...
	if _, err := DecodeTabular(strings.NewReader("r[1|]{a}:\n  \"a\\qb\"\n")); err == nil {
...
func TestAnUnquotedColonInAValueIsAnError(t *testing.T) {
	...
	_, err := DecodeTabular(strings.NewReader(src))
	if err == nil {
...
func TestLengthMarkerDecodes(t *testing.T) {
	...
	doc, err := DecodeTabular(strings.NewReader("findings[#2]{a}:\n  x\n  y\n"))
	if err != nil {
...
func TestABlankLineBetweenRowsIsSkipped(t *testing.T) {
	...
	doc, err := DecodeTabular(strings.NewReader("r[2|]{a}:\n  x\n\n  y\n"))
	if err != nil {
...
func TestALongRowIsStillAnError(t *testing.T) {
	...
	if _, err := DecodeTabular(strings.NewReader("r[1|]{a|b}:\n  1|2|3\n")); err == nil {
...
func TestDecodeAnyReadsALeadingScalarDocument(t *testing.T) {
	...
	// DecodeTabular is tabular-only by contract and rejects it; Decode is the
	// general entry point.
	//
	...
	// output when that consumer is migrated off toon.
	src := "path: /tmp/x\nsize: 12\nis_dir: false\n"
	...
	if !ok {
		t.Fatalf("Decode returned %T, want a map", doc)
	}
...
func TestTabularSiblingStillReachesMeta(t *testing.T) {
	...
	doc, err := DecodeTabular(strings.NewReader("entries[2,]{name,type}:\n  a,dir\n  b,file\ntotal: 6\n"))
	if err != nil {
		t.Fatalf("DecodeTabular: %v", err)
	}
...
func TestEmptyInputIsStillAnError(t *testing.T) {
	...
	if _, err := DecodeTabular(strings.NewReader("")); err == nil {
