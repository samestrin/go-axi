...
// unexported and DecodeTabular returns map[string]any, so the declared field ORDER,
...
//
// It is the dispatch point between DecodeTabular and Decode, and it deliberately
...
//
// NOT the same test as the unexported isTabularHeader in check.go, and the two
// must not be merged. That one asks "does this WHOLE document contain a header
// with a braced field list", over []byte, to set Verdict.Tier on output that
// go-axi itself just encoded. This one asks "does this ONE line open a tabular
// array", over a string, to route a line the caller is about to read. The
// braced field list is the difference that matters: `findings[0]:` is a
// legitimate empty array here and is deliberately not a tabular header there,
// because an empty array is not what the tier is describing.
