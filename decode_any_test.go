...
// IsTabularHeader is the dispatch point between DecodeTabular and Decode, so it
...
func TestIsTabularHeader(t *testing.T) {
	for name, tc := range map[string]struct {
		...
		// whole point: the test is structural, so DecodeTabular gets to report the
...
func TestDecodeAnyFileReadsADocument(t *testing.T) {
	...
	if !ok {
		t.Fatalf("DecodeFile returned %T, want a map", doc)
	}
...
func TestDecodeAnyFileNamesAMissingFile(t *testing.T) {
	_, err := DecodeFile(filepath.Join(t.TempDir(), "nope.axi"))
	if err == nil {
...
func TestDecodeReadsAnUnnamedArray(t *testing.T) {
	...
	doc, err := DecodeTabular(strings.NewReader("[2|]{a|b}:\n  1|2\n  3|4\n"))
	if err != nil {
...
func TestAnUnsupportedDelimiterIsRejected(t *testing.T) {
	...
	_, err := DecodeTabular(strings.NewReader("r[1;]{a;b}:\n  1;2\n"))
	if err == nil {
...
func TestAQuotedArrayNameDecodes(t *testing.T) {
	...
	// decoded document".
	doc, err := DecodeTabular(strings.NewReader("\"my array\"[1|]{a|b}:\n  1|2\n"))
	if err != nil {
