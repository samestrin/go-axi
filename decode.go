// This file reads TOON (Token-Optimized Object Notation) payloads.
//
...
//
// The codec is github.com/toon-format/toon-go, and DECODING is still its job:
// Decode forwards the whole document to it. DecodeTabular is the part toon-go
// does not cover — it reads the header, isolates one array, and hands the codec
// a document it can accept.
//
...
// MaxDocumentBytes bounds a whole document read by Decode.
//
// toon-go's decoder takes a []byte, so the document has to be resident — but
// "resident" is not "unbounded". Decode accepts any io.Reader, including one
...
func DecodeTabular(r io.Reader) (*Document, error) {
	...
	if headerLine == "" {
		return nil, fmt.Errorf("toon: no header found: expected a line like " +
			"`name[N|]{field|field}:` as the first non-blank line")
	}
...
// DecodeTabularFile reads one TOON tabular array from a file, naming it on error.
func DecodeTabularFile(path string) (*Document, error) {
	f, err := os.Open(path)
	if err != nil {
		return nil, err
	}
	defer func() { _ = f.Close() }()
	doc, err := DecodeTabular(f)
	if err != nil {
...
// Decode reads a TOON document of any shape, preserving toon-go's types.
//
// DecodeTabular is tabular-only by contract and rejects anything else, which is
...
// DecodeFile reads a TOON document of any shape from a file.
