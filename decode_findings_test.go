...
// --- HIGH (mira): io.ReadAll in Decode is unbounded

func TestDecodeAnyRefusesAnOversizedDocument(t *testing.T) {
	// Decode is the public entry point behind `toon parse --shape document`,
...
func TestDecodeAnyAcceptsADocumentAtTheLimit(t *testing.T) {
	...
	doc, err := Decode(strings.NewReader("k: " + strings.Repeat("x", 64*1024) + "\n"))
	if err != nil {
...
func TestDecodeAnyRejectsAnEmptyTopLevelArray(t *testing.T) {
	...
	if _, err := Decode(strings.NewReader("[0]:\n")); err == nil {
...
func TestABlankLineBetweenRowsShiftsReportedLineNumbers(t *testing.T) {
	...
	_, err := DecodeTabular(strings.NewReader("r[3|]{a|b}:\n  1|2\n\n  3|4|5\n"))
	if err == nil {
